...
- **Source or packaging fix**: make a new patch release. There is no package-revision scheme, and tags are never moved or reused.
...
1. **`build`**, `runs-on: ubuntu-latest` in `container: ubuntu:22.04`.
   ...
   - `dpkg-deb -f <deb> Depends` is printed and checked: it must name the ALSA library with a version constraint and `libc6 (>= …)`.
   - The tarball (§8.2).
...
2. **`install-deb`**, which `needs: build`, in a matrix of `container:` `debian:12`, `debian:13`, `ubuntu:22.04`, `ubuntu:24.04`, `ubuntu:26.04`, `fail-fast: false`.
   ...
   - `apt-get update && apt-get install -y ./tenuto_<ver>-1_amd64.deb`, which resolves ALSA from the distro's own archive.
   - `dpkg -s tenuto` must report `Status: install ok installed`.
   - `scripts/release/smoke-test.sh /usr/bin/tenuto <ver>`.
...
3. **`install-tarball`**, which `needs: build`, in the same five-container matrix.
   - Installs only the ALSA runtime library, named per matrix entry (`libasound2` on Debian 12 and Ubuntu 22.04, `libasound2t64` on Debian 13, Ubuntu 24.04 and 26.04). No toolchain, no `-dev` packages.
   - Verifies checksums, extracts the tarball and runs `smoke-test.sh` on the extracted `tenuto`.

No test container installs Rust or rebuilds anything.
...
1. **`validate`** always runs, on both events. Checkout has `fetch-depth: 0`. Outputs: `sha`, `version`.
   ...
   - Tag push: `scripts/release/check-tag.sh "$GITHUB_REF_NAME" "$version"` (the tag must be exactly `v` + the `Cargo.toml` version at the tagged commit); the commit is an ancestor of `origin/main`; `scripts/release/find-rehearsal.sh "$sha"` finds a qualifying rehearsal (§7.2). This job gets `actions: read` for the lookup.
...
It re-filters the result in `jq` on every field, so a server-side filter that stopped working cannot let a wrong run through: `workflowDatabaseId` equals the ID of `release.yml` (from `gh workflow view release.yml --json id`), `event == "workflow_dispatch"`, `headBranch == "main"`, `headSha == $sha`, `conclusion == "success"`. Exit 0 and print the run URL if at least one run matches. Otherwise exit 1 with a message saying which rehearsal is missing. The title is never matched on.
...
### 9.1 `scripts/release/smoke-test.sh <tenuto> <expected-version>`

`set -euo pipefail`. Creates a temporary `$root`, with a `trap` that removes it on exit. Inside it are `$root/home` (the isolated directory) and `$root/out` (captured output, kept outside the directory whose emptiness is checked). Each command runs with the ambient `PATH` and `HOME=$root/home`, `XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME` all under `$root/home`.
...
This establishes installation and startup compatibility: the loader resolves every `NEEDED` library at a compatible version, and the binary runs its CLI. It does not test audio output. Containers have no sound device, and playback stays covered by the existing test suite.
...
Plain shell tests with fixtures under `scripts/release/fixtures/`. `gh` is stubbed by a script put first on `PATH` that returns fixture JSON or exit codes. They cover:

- `check-tag.sh`: match; missing `v`; pre-release suffix; wrong version.
- `find-rehearsal.sh`: a qualifying run; runs that fail on each field in turn (other workflow ID, `push` event, other branch, other SHA, `failure` conclusion); an empty list; `gh` failing (must fail, not pass).
...
`ci.yml` runs them in the `release-scripts` job.
...
- **`README.md` Install**: add "Prebuilt packages (x86_64)", pointing to GitHub Releases. The `.deb` is "tested on Debian 12 and 13; Ubuntu 22.04, 24.04 and 26.04" (`sudo apt install ./tenuto_<ver>-1_amd64.deb`). The tarball lists the inspected runtime libraries and glibc version, with `sha256sum -c SHA256SUMS`. `cargo install tenuto` and the from-source steps stay.
